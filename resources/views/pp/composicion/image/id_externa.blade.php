    ...
    methods: {
      async submitFile() {
        ...
        */
        ...
        let url = "{{url('api/compoExternas')}}-{{$id}}/image";
        await axios({
          method: 'post',
          url: url,
          headers: {
      ...
      async load() {
        try {
          ...
          try {
            await Promise.all([self.GET_DATA('api/compoExternas/{{$id}}'),self.GET_DATA('api/tipoarchivos')]).then((v) => {
            self.model = v[0]
            self.data = v[1]
          })
          } catch (e) {
        ...
        swalWithBootstrapButtons({
          ...
          if (result.value) {
            try {
              ...
              let url = "{{url('api/compoExternas-image-delete')}}/" + id
