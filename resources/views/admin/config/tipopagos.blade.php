                    ...
                    <div class="page-header">
                        <div class="page-title">
                            <p><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                                ...
                                </svg> Configuración / Métodos de pago</p>
                        </div>
        ...
        <script type="module">
            ...
            createApp({
                ...
                methods: {
                    async Save() {
                        try {
                            const urlBase = "{{ url('api/tipopagos') }}";
                            let res;

                            if (this.add === false) {
                                const url = `${urlBase}/${this.model.id}`;
                                res = await axios.put(url, this.model);
                            } else {
                                res = await axios.post(urlBase, this.model);
                            }

                            // Éxito: refrescamos y cerramos modal
                            dt.destroy();
                            await this.load();
                            dt.create();
                            ...
                            swal({
                                title: 'Guardado',
                                text: 'El método de pago se guardó correctamente.',
                                type: 'success',
                                padding: '2em'
                            });

                        } catch (e) {
                            // Si es validación de Laravel (422), mostramos los mensajes
                    ...
                    async load() {
                        ...
                        try {
                            ...
                            try {
                                await Promise.all([self.GET_DATA("{{ url('api/tipopagos') }}")]).then((v) => {
                                    self.data = v[0]
                                })
