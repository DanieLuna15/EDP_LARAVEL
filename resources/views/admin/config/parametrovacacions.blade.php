        ...
        <div class="page-header">
            <div class="page-title">
                <p><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-grid">
                    ...
                    </svg> Configuración / Parametros de vacaciones</p>
            </div>
            ...
            <div class="modal-body">
              <div class="form-row mb-4">
                <div class="form-group col-md-12">
                  <label for="inputEmail4">Desde (*años)</label>
                  <input type="text" v-model="model.desde" class="form-control" placeholder="Desde">
                </div>
                <div class="form-group col-md-12">
                  <label for="inputEmail4">Hasta (*años)</label>
                  <input type="text" v-model="model.hasta" class="form-control" placeholder="Hasta">
                </div>
                <div class="form-group col-md-12">
                  <label for="inputEmail4">Dias (vacacion)</label>
                  <input type="text" v-model="model.dias" class="form-control" placeholder="Dias">
                </div>
            ...
            methods: {
                async Save() {
                    try {
                        ...
                        let url = "{{url('api/parametrovacacions')}}";
                        if (this.add == false) {
                            url = "{{url('api/parametrovacacions')}}/"+this.model.id
                ...
                async load() {
                    try {
                        ...
                        try {
                            await Promise.all([self.GET_DATA("{{url('api/parametrovacacions')}}")]).then((v) => {
                                self.data = v[0]
                            })
                    ...
                    swalWithBootstrapButtons({
                        ...
                        if (result.value) {
                            try {
                                ...
                                let url = "{{url('api/parametrovacacions')}}/"+id
