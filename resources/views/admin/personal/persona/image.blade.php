      ...
      <div class="col-lg-6 col-12">
        ...
        <div class="table-responsive mb-4 mt-4">
            <table id="table_dt" class="table table-hover non-hover" style="width:100%">
              ...
              <tbody>
                <tr v-for="(m,i) in model.file_personas">
                  <td>{{i+1}}</td>
                  <td><div class="media">
    ...
    methods: {
      async submitFile() {
        ...
        */
        ...
        let url = "{{url('api/personas')}}-{{$id}}/image";
        await axios({
          method: 'post',
          url: url,
          headers: {
      ...
      async load() {
        try {
          ...
          try {
            await Promise.all([self.GET_DATA('api/personas/{{$id}}'),self.GET_DATA('api/tipoarchivos')]).then((v) => {
            self.model = v[0]
            self.data = v[1]
          })
          } catch (e) {
        ...
        swalWithBootstrapButtons({
          ...
          if (result.value) {
            try {
              ...
              let url = "{{url('api/personas-image-delete')}}/" + id
