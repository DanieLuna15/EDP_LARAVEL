@component('application')
    @slot('body')
        @verbatim
            <div id="block_ui">
                <div class="row justify-content-center">
                    <div class="col-lg-12 col-12">
                        <div class="widget-content widget-content-area br-6">
                            <div class="section general-info">
                                <div class="info">
                                    <h6 class="">Seleccione la Consolidacion para poder realizar en Pago</h6>
                                    <div class="row">

                                        <div class="col-3">
                                            <div class="form-group ">
                                                <label>Lotes</label>

                                                <div class="input-group mb-4">

                                                    <select v-model="lote" class="form-control">
                                                        <template v-for="(m, i) in consolidacions.reverse()">
                                                            <option v-if="m.consolidacion_pago_detalle == null"
                                                                :value="m">
                                                                {{ m . fecha }} - Consolidación: {{ m . id }} - Lote: {{ m . compra . nro_compra }}
                                                            </option>
                                                        </template>
                                                    </select>
                                                    <button class="btn btn-success " @click="GetLote()" type="button"><svg
                                                            xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                            viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                            stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                                            class="feather feather-send">
                                                            <line x1="22" y1="2" x2="11" y2="13">
                                                            </line>
                                                            <polygon points="22 2 15 22 11 13 2 9 22 2"></polygon>
                                                        </svg></button>
                                                </div>
                                            </div>
                                        </div>

                                        <div class="col-12">
                                            <hr>
                                            <div class="row">
                                                <div class="col-12">
                                                    <div class="table-responsive mb-4 mt-4">
                                                        <table id="table_dt" class="table table-hover non-hover"
                                                            style="width:100%">
                                                            <thead>
                                                                <tr>
                                                                    <th>Fecha</th>
                                                                    <th>Peso Total </th>
                                                                    <th>Valor Total </th>
                                                                </tr>
                                                            </thead>
                                                            <tbody>
                                                                <tr v-for="(m,i) in lotes">
                                                                    <td>{{ m . fecha }}</td>
                                                                    <td>{{ m . peso_total }}</td>
                                                                    <td>{{ m . valor_total }}</td>



                                                                </tr>
                                                            </tbody>

                                                        </table>
                                                    </div>
                                                </div>
                                            </div>

                                        </div>


                                        <div class="col-sm-2 col-12 m-4">
                                            <div class="form-group">
                                                <label for="fullName">Peso Total</label>
                                                <input :value="Number(SumPeso).toFixed(2)" disabled type="text"
                                                    class="form-control mb-4">
                                            </div>
                                        </div>

                                        <div class="col-sm-2 col-12 m-4">
                                            <div class="form-group">
                                                <label for="fullName">Valor Total</label>
                                                <input :value="Number(SumValor).toFixed(2)" disabled type="text"
                                                    class="form-control mb-4">
                                            </div>
                                        </div>





                                        <div class="col-12">
                                            <div class="row">

                                                <div class="col-12">
                                                    <div class="row">
                                                        <div class="col-3 ">
                                                            <label for="">Tipo de pago</label>
                                                            <select class="form-control" v-model="pago.tipo"
                                                                @change="pago.adelanto=pago.tipo==1?SumValor:0">
                                                                <option value="1">CONTADO</option>
                                                                <option value="2">CREDITO</option>
                                                            </select>
                                                        </div>
                                                        <div class="col-3 ">
                                                            <div class="form-group">
                                                                <label for="fullName">Adelanto</label>
                                                                <input value="" type="text" v-bind:disabled="pago.tipo==1"
                                                                    v-model.number="pago.adelanto" class="form-control mb-4">
                                                            </div>
                                                        </div>
                                                        <div class="col-3 ">
                                                            <label for="">Bancos</label>
                                                            <select class="form-control" v-model="pago.banco_id">
                                                                <template v-for="(m,i) in bancos">

                                                                    <option :value="m.id">{{ m . name }} -
                                                                        {{ m . titular }} - {{ m . cuenta }}</option>


                                                                </template>
                                                            </select>
                                                        </div>
                                                        <div class="col-3 ">
                                                            <div class="form-group">
                                                                <label for="fullName">Fecha limite de Pago</label>
                                                                <input value="" v-model="pago.fecha"
                                                                    v-bind:disabled="pago.tipo==1" type="date"
                                                                    class="form-control mb-4">
                                                            </div>
                                                        </div>
                                                        <div class="col-6 ">
                                                            <label for="">Formas de Pago</label>
                                                            <select class="form-control" v-model="pago.formapago_id">
                                                                <template v-for="(m,i) in formapagos">

                                                                    <option :value="m.id">{{ m . name }}</option>


                                                                </template>
                                                            </select>
                                                        </div>
                                                        <div class="col-6 ">
                                                            <div class="form-group">
                                                                <label for="fullName">Observaciones</label>
                                                                <input value="" type="text" v-model="pago.observaciones"
                                                                    class="form-control mb-4">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>

                                        <div class="col-sm-12 col-12">
                                            <hr>
                                        </div>

                                        <div class="col-6">
                                            <button class="btn btn-dark w-100" @click="back()">Regresar</button>
                                        </div>
                                        <div class="col-6">
                                            <button class="btn btn-success w-100" @click="Save()">Guardar Pago</button>
                                        </div>

                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        @endverbatim
    @endslot
    @slot('script')
        <script type="module">
            import Table from "{{ asset('config/dt.js') }}"
            import Block from "{{ asset('config/block.js') }}"


            const {
                createApp
            } = Vue
            let block = new Block()

            createApp({
                data() {
                    return {
                        user: {},
                        sucursal: {},
                        model: {
                            inactivo: 1,
                            pagar: 1,
                            camion: '',
                            placa: '',
                            chofer: '',
                            e_recepcion: '',
                            e_despacho: '',

                            fecha: '',
                            hora: '',
                            almacen_id: '',
                            compra_id: '',

                            medidas: []
                        },
                        producto_model: {
                            nro: 0,
                            peso: 0,
                            cantidad: 1,
                        },
                        producto: {
                            name: '',
                            medida_productos: []
                        },
                        paramConsolidacion: {

                        },
                        medida_producto: {

                        },
                        pago: {
                            tipo: 2,
                            fecha: "",
                            adelanto: 0,
                            banco_id: 1,
                            formapago_id: 1,
                            observaciones:"SN"
                        },
                        lote: null,
                        consolidacions: [],
                        lotes: [],
                        consolidacionParams: [],
                        medidas: [],
                        bancos: [],
                        almacens: [],
                        data: [],
                        productos: [],
                        productos_model: [],
                        submedida: {
                            name: '',
                            valor_1: 0,
                            valor_2: 0,
                            categoria: {
                                name: ''
                            }
                        },
                        formapagos: []

                    }
                },
                computed: {
                    categoriaList() {
                        if (!this.lote) {

                            return []
                        }

                        return this.lote.category_list.map((c) => {
                            let porcentaje = Number(this.paramConsolidacion.monto) / 100
                            c.ajuste = Number(c.sumaTotal * porcentaje)
                            c.nuevo_peso = c.sumaTotal - c.ajuste
                            c.nuevo_peso_2 = c.nuevo_peso - c.criterio
                            c.oficial = c.nuevo_peso_2 - c.nuevoajuste
                            c.precioCompra = c.oficial * c.precio
                            return c
                        })
                    },
                    SumPeso() {
                        return this.lotes.reduce((a, b) => a + Number(b.peso_total), 0)
                    },
                    SumValor() {
                        return this.lotes.reduce((a, b) => a + Number(b.valor_total), 0)
                    },

                },
                methods: {
                    async GET_DATA(path) {
                        try {
                            let res = await axios.get("{{ url('api') }}/" + path)
                            return res.data
                        } catch (e) {

                        }
                    },
                    async GetLote() {
                        block.block();
                        try {
                            let buscar = this.lotes.filter((l) => l.id == this.lote.id)
                            if (buscar.length == 0) {
                                let lote = {
                                    ...this.lote
                                }
                                this.lotes.push(lote)

                            }
                        } catch (e) {

                        } finally {
                            block.unblock();
                        }
                    },
                    AgregarProducto() {
                        let medidas = this.medida_producto
                        let bruto = Number(this.producto_model.peso) / Number(this.producto_model.nro)
                        let neto = Number(Number(this.producto_model.peso) - (Number(medidas.medida.retraccion) *
                            Number(this.producto_model.cantidad)))
                        let sub = Number(Number(this.producto_model.peso) - (Number(medidas.medida.retraccion) * Number(
                            this.producto_model.cantidad))) / Number(this.producto_model.nro)
                        let unit = {}
                        medidas.sub_medidas.map((a) => {
                            let valor_1 = Number(a.valor_1)
                            let valor_2 = Number(a.valor_2)
                            if (sub >= valor_1 && sub <= valor_2) {
                                unit = a
                            }
                        })
                        const producto = {
                            sub_medida: unit,
                            producto: {
                                name: this.producto.name,
                                id: this.producto.id,
                            },
                            categoria: {},
                            medida_producto: medidas,
                            producto_model: {
                                peso: this.producto_model.peso,
                                neto: neto,
                                nro: this.producto_model.nro,
                                cantidad: this.producto_model.cantidad,
                            },

                        }
                        this.productos_model.push(producto)
                    },
                    ChangeProducto(i) {
                        let medidas = this.productos_model[i].medida_producto
                        this.productos_model[i].producto_model.nro = this.productos_model[i].medida_producto.valor *
                            this.productos_model[i].producto_model.cantidad

                        let bruto = Number(this.productos_model[i].producto_model.peso) / Number(this.productos_model[i]
                            .producto_model.nro)
                        let neto = Number(Number(this.productos_model[i].producto_model.peso) - (Number(medidas.medida
                            .retraccion) * Number(this.productos_model[i].producto_model.cantidad)))
                        let sub = Number(Number(this.productos_model[i].producto_model.peso) - (Number(medidas.medida
                            .retraccion) * Number(this.productos_model[i].producto_model.cantidad))) / Number(this
                            .productos_model[i].producto_model.nro)
                        let unit = {}
                        medidas.sub_medidas.map((a) => {
                            let valor_1 = Number(a.valor_1)
                            let valor_2 = Number(a.valor_2)
                            if (sub >= valor_1 && sub <= valor_2) {
                                unit = a
                            }
                        })

                        this.productos_model[i].producto_model.neto = neto
                        this.productos_model[i].sub_medida = unit
                    },
                    CambioProducto() {

                        this.medida_producto = this.producto.medida_productos[0]
                        this.producto_model.nro = this.producto_model.cantidad * Number(this.medida_producto.valor)
                    },
                    AgregarSubmedida(i) {
                        const submedida = {
                            name: this.submedida.name,
                            valor_1: this.submedida.valor_1,
                            valor_2: this.submedida.valor_2,
                            categoria: this.submedida.categoria,
                        }
                        this.model.medidas[i].submedidas.push(submedida)
                    },
                    async Save() {
                        block.block();
                        try {
                            this.pago = this.pago
                            this.pago.user_id = this.user.id
                            this.pago.sucursal_id = this.sucursal.id
                            this.pago.valor_total = this.SumValor
                            this.pago.lotes = this.lotes
                            let res = await axios.post("{{ url('api/consolidacionAvePagos') }}", this.pago)
                            const swalWithBootstrapButtons = swal.mixin({
                                confirmButtonClass: 'btn btn-success btn-rounded',
                                cancelButtonClass: 'btn btn-danger btn-rounded mr-3',
                                buttonsStyling: false,
                            })
                            let consolidacion = res.data
                            swalWithBootstrapButtons({
                                title: 'Pago de Consolidacion Guardada',
                                text: "Su Consolidacion fue guardada",
                                type: 'success',
                                showCancelButton: true,
                                confirmButtonText: 'IMPRIMIR PDF',
                                cancelButtonText: 'REGRESAR',
                                reverseButtons: true,
                                padding: '2em'
                            }).then(async (result) => {
                                if (result.value) {
                                    try {
                                        window.open(consolidacion.url_pdf, '_blank');
                                        this.back()
                                    } catch (e) {
                                    }
                                } else {
                                    this.back()
                                }
                            })

                        } catch (e) {

                        } finally {
                            block.unblock();
                        }
                    },
                    back() {
                        window.location.replace(document.referrer);
                    }
                },
                mounted() {
                    this.$nextTick(async () => {
                        let self = this;
                        block.block();
                        try {
                            await Promise.all([
                                self.GET_DATA('consolidacions_aves'),
                                self.GET_DATA('consolidacionParams'),
                                self.GET_DATA('productos'),
                                self.GET_DATA('almacens'),
                                self.GET_DATA('bancos'),
                                self.GET_DATA('formapagos')
                            ]).then((v) => {
                                self.consolidacions = v[0] ||
                            []; // Verificar que los datos estén disponibles
                                self.consolidacionParams = v[1] || [];
                                self.productos = v[2] || [];
                                self.almacens = v[3] || [];
                                self.bancos = v[4] || [];
                                self.formapagos = v[5] || [];
                            });

                            // Verificación para evitar acceder a un índice fuera de rango
                            if (self.consolidacionParams.length > 0) {
                                self.paramConsolidacion = self.consolidacionParams[0];
                            } else {
                                console.warn('No hay parámetros de consolidación disponibles');
                            }

                            if (self.bancos.length > 0) {
                                self.pago.banco_id = self.bancos[0].id;
                            } else {
                                console.warn('No hay bancos disponibles');
                            }

                        } catch (e) {
                            console.error('Error al cargar los datos:', e);
                        } finally {
                            let login = localStorage.getItem('AppUser');
                            if (login != null) {
                                let login_user = JSON.parse(login);
                                self.user = login_user;
                            }

                            let sucursal = localStorage.getItem('AppSucursal');
                            if (sucursal != null) {
                                self.sucursal = JSON.parse(sucursal);
                            }

                            block.unblock();
                        }
                    });
                }

            }).mount('#meApp')
        </script>
    @endslot
    @slot('style')
        <style>
            .fm-ct {
                width: 100px;
            }

            td.td-c {
                width: 100px;
            }
        </style>
    @endslot
@endcomponent
