  ...
  createApp({
    ...
    methods: {
      ...
      async GetLote() {
        block.block();
        try {
          let res = await axios.get("{{url('api')}}/consolidacionAvePagos/" + this.model.compra_id)
          this.lote = res.data
        } catch (e) {
      ...
      async Save() {
        ...
        try {
          ...
          let res = await axios.post("{{url('api/consolidacionAvePagoTickets')}}",this.pagar)
    ...
    mounted() {
      this.$nextTick(async () => {
        ...
        try {
          ...
          await Promise.all([self.GET_DATA('consolidacionAvePagos'), self.GET_DATA('consolidacionparams'), self.GET_DATA('productos'), self.GET_DATA('almacens'),
