            ...
            <div class="row">
              ...
              <div class="col-sm-2 col-12">
                <div class="form-group">
                  ...
                  <input v-model="model.fecha" disabled id="datePicker" type="date" class="form-control mb-4">
                </div>
              </div>
              <div class="col-sm-2 col-12">
                <div class="form-group">
                  <label for="fullName">Fecha Salida:</label>
                  <input v-model="model.fecha_salida" id="datePicker1" type="date" class="form-control mb-4">
                </div>
              </div>
              <div class="col-sm-2 col-12">
                <div class="form-group">
                  <label for="fullName">Fecha Llegada:</label>
                  <input v-model="model.fecha_llegada" id="datePicker2" type="date" class="form-control mb-4">
                </div>
              </div>
              <div class="col-4">
                <div class="form-group ">
                ...
                <div class="form-group ">
                  ...
                  <div class="input-group mb-4">
                    <select v-model="model.proveedor_compra" class="form-control">
                      ...
                      <option v-for="(m,i) in proveedors" :value="m">{{m.abreviatura}} - {{m.nombre}}</option>
              ...
              <div class="col-sm-3 col-12">
                <div class="form-group">
                  ...
                  <select name="" id="" class="form-control" :value="cintaSelected" v-model="cinta" @change="CambiarCinta" :disabled="producto.tipo==0">
                    <template v-for="(c,e) in medida_producto.sub_medidas" >
                        <option v-if="cintaSelected==c.name || c.name=='AMARILLA'" :value="c.name">{{c.name}}</option>
                    </template>
              ...
              <div class="col-sm-3 col-12">
                <div class="form-group">
                  ...
                  <input v-model="producto_model.nro" :disabled="producto.tipo==0" type="text" class="form-control mb-4">
                </div>
              </div>
              <div class="col-sm-3 col-12">
                <div class="form-group">
                  <label for="fullName">Pigmento</label>
                  <select name="" id="" class="form-control" v-model="producto_model.pigmento">
              ...
              <div class="col-sm-3 col-12">
                <div class="form-group">
                  ...
                  <select name="" id="" class="form-control" v-model="producto_model.tipo">
                  ...
                  <table id="table_dt" class="table table-hover non-hover" style="width:100%">
                    ...
                    <tbody>
                      <tr v-for="(m,i) in productos_model_computed">
                        ...
                        <td class="td-c"> <input  v-if="m.tipo==1" v-model.number="m.producto_model.nro" @change="ChangeProducto(i)" type="text" class="fm-ct form-control "></td>
                        <td class="td-c">
                            <span  v-if="m.tipo==1">
                        ...
                        <td> <select v-model="m.sub_medida" class="form-control"  v-if="m.tipo==1">
                        ...
                        <td>
                             <select v-model="m.sub_medida_2" class="form-control" @change="AlertaCambioCintaOriginal" v-if="m.tipo==1">
                        ...
                        <td class="text-center"><svg @click="eliminarProducto(i)" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-trash-2 icon text-danger">
                    ...
                    <tfoot>
                      <tr class="table-default">
                        ...
                        <td colspan="6" class="text-left bg-white text-black">{{SumCantPollos}}</td>
                      </tr>
                      <tr class="table-default">

                        <td colspan="6">Cantidad de Cajas</td>

                        <td colspan="6" class="text-left bg-white text-black">{{SumCantEnvases}}</td>
                      </tr>
                      <tr class="table-default">

                        <td colspan="6">Peso Bruto Total</td>

                        <td colspan="6" class="text-left bg-white text-black">{{Number(SumPesoBruto).toFixed(2)}}</td>
                      </tr>
                      <tr class="table-primary">

                        <td colspan="6">Retraccion de Tara</td>

                        <td colspan="6" class="text-left bg-white text-black">-{{Number(SumRetraccion).toFixed(2)}}</td>
                      </tr>
                      <tr class="table-default">

                        <td colspan="6">Peso Neto Total</td>

                        <td colspan="6" class="text-left bg-white text-black">{{Number(SumPesoNeto).toFixed(2)}}</td>
                      </tr>
...
<script type="module">
  ...
  createApp({
    ...
    computed: {
      ...
      SumPesoNeto() {
        return this.productos_model.reduce((a, b) => a + Number(Number(b.producto_model.peso) - (Number(b.medida_producto.medida.retraccion) * Number(b.producto_model.cantidad))), 0)
      },
      SumRetraccion() {
        return this.productos_model.reduce((a, b) => a + (Number(b.medida_producto.medida.retraccion) * Number(b.producto_model.cantidad)), 0)
      },
      ProductosSinCinta(){
        return this.productos_model.filter(e=>(e.sub_medida.hasOwnProperty('id') == false&&e.tipo==1) ||( e.sub_medida_2.hasOwnProperty('id') == false && e.tipo==1))
      },
      cintaSelected(){
    ...
    mounted() {
      this.$nextTick(async () => {
        ...
        try {
          ...
          await Promise.all([self.GET_DATA('categorias'), self.GET_DATA('medidas'), self.GET_DATA('productos'), self.GET_DATA('almacens'), self.GET_DATA('proveedorCompras'), self.GET_DATA('cajaInventarios')]).then((v) => {
            self.categorias = v[0]
            self.medidas = v[1]
            self.productos = v[2]
            self.almacens = v[3]
            ...
            if (self.cajas.length) {
              ...
              let filters = self.cajas.filter((c) => c.almacen_id == self.model.almacen_origen_id)
              if (filters.length) {
