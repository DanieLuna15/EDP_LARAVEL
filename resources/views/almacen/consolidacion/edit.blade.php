      ...
      <div class="widget-content widget-content-area br-6">
        <div class="section general-info">
          <div class="info">
            ...
            <div class="col-3">
              ...
              <div class="col-12">
                ...
                <div class="row">
                  ...
                  <div v-for="(m,i) in categoriaList" class="col-xxl-10 col-xl-12 col-lg-10 col-md-10 col-sm-10 mx-auto">

                    <div class="card">
                      <div class="card-body">
                        <h6>{{m.categoria.name}} - Lote N°: {{m.compra.nro}} Fecha:{{m.compra.fecha}} - Proveedor: {{m.compra.proveedor_compra.nombre}} {{m.compra.proveedor_compra.abreviatura}}</h6>
                        <div class="row">
                          <div class="col-12">
                            <div class="table-responsive mb-4 mt-4">
                              <table id="table_dt" class="table table-hover non-hover" style="width:100%">
                                ...
                                <tbody>
                                  <tr>
                                    <td class="td-c"> <input v-model.number="m.suma_total" type="text" class="fm-ct form-control "></td>
                                    ...
                                    <td class="td-c"> <input v-model.number="m.precio" :disabled="m.n_cambios>=6" type="text" class="fm-ct form-control "></td>
                                    <td class="td-c">{{Number(m.precioCompra).toFixed(2)}}</td>
                                    <td class="td-c">{{Number(m.pollos).toFixed(2)}}</td>
                                    <td class="td-c"> {{Number(m.ajuste).toFixed(2)}}</td>
            ...
            <div class="col-6">
              <button class="btn btn-success w-100" @click="Save()">ASIGNAR PRECIO / EDITAR PRECIO</button>
            </div>
...
<script type="module">
  ...
  createApp({
    ...
    computed: {
      ...
      SumValor() {
        let consolidationSum = this.categoriaList.reduce((a, b) => a + Number(b.precioCompra), 0);
    ...
    methods: {
      ...
      async Save() {
        ...
        try {
          ...
          let res = await axios.put("{{url('api/consolidacions')}}/{{$id}}", this.model)
    ...
    mounted() {
      this.$nextTick(async () => {
        ...
        try {
          ...
          await Promise.all([self.GET_DATA('compras'), self.GET_DATA('consolidacionparams'), self.GET_DATA('productos'), self.GET_DATA('almacens'), self.GET_DATA('bancos'),self.GET_DATA("consolidacions/{{$id}}")]).then((v) => {
            self.compras = v[0]
