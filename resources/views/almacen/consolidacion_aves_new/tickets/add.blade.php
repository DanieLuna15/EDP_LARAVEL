                              ...
                              <table id="table_dt" class="table table-hover non-hover" style="width:100%">
                                ...
                                <tbody>
                                  <tr  v-for="(m,i) in categoriaList">
                                    <td class="td-c">{{m.consolidacion.fecha}} </td>
                                    <td class="td-c">{{m.consolidacion.id}} </td>
  ...
  createApp({
    ...
    methods: {
      ...
      async GetLote() {
        block.block();
        try {
          let res = await axios.get("{{url('api')}}/consolidacionAveNewPagos/" + this.model.compra_id)
          this.lote = res.data
        } catch (e) {
      ...
      async Save() {
        ...
        try {
          ...
          let res = await axios.post("{{url('api/consolidacionAveNewPagoTickets')}}",this.pagar)
    ...
    mounted() {
      this.$nextTick(async () => {
        ...
        try {
          ...
          await Promise.all([self.GET_DATA('consolidacionAveNewPagos'), self.GET_DATA('consolidacionparams'), self.GET_DATA('productos'), self.GET_DATA('almacens'),
