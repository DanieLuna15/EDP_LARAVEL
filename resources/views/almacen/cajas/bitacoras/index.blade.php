            ...
            <table id="table_dt" class="table table-hover non-hover" style="width:100%">
              ...
              <tbody>
                <tr v-for="(m,i) in data">
                  <td>{{m.id}}</td>
                  <td>{{m.caja.name}}</td>
                  <td>{{m.caja_inventario.cantidad}}</td>
                  <td>{{m.caja_inventario.almacen.name}}</td>
                  <td>{{m.compra.nro}}</td>
                  <td>{{m.compra.fecha}}</td>
                  <td>{{m.compra.fecha_salida}}</td>
                  <td>{{m.compra.fecha_llegada}}</td>
                  <td>{{m.compra.proveedor_compra.nombre}}</td>
                  <td>{{m.compra.camion}}</td>
                  <td>{{m.compra.placa}}</td>
                  <td>{{m.compra.chofer}}</td>
                  <td>{{m.compra.sum_peso_bruto}}</td>
                  <td>{{m.compra.sum_peso_neto}}</td>
                  <td>{{m.compra.e_recepcion}}</td>
                  <td>{{m.compra.e_despacho}}</td>
                  <td>{{m.compra.user.nombre}}</td>
            ...
            methods: {
                ...
                async load() {
                    try {
                        ...
                        try {
                            await Promise.all([self.GET_DATA("{{url('api/cajaCompras')}}")]).then((v) => {
                                self.data = v[0]
                            })
