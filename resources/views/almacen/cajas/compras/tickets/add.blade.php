            ...
            <div class="row">
              ...
              <div class="col-6">
                <div class="form-group ">
                  <label>Compras de caja</label>

                  <div class="input-group mb-4">

                    <select v-model="model.compra_id" class="form-control">

                      <template v-for="(m,i) in compraCajas">

                        <option v-if="m.tipo==2&&m.monto!=m.total" :value="m.id">{{m.fecha}} - Total: {{m.total}} - Pendiente: {{m.total-m.monto}}</option>
              ...
              <div class="col-12">
                <hr>
                <div v-if="!lote.hasOwnProperty('monto')" class="alert alert-warning alert-dismissible fade show mb-4" role="alert">
                        ...
                        <div class="col-12">
                          <div class="table-responsive mb-4 mt-4">
                            <table id="table_dt" class="table table-hover non-hover" style="width:100%">
                              ...
                              <tbody>
                                <tr v-for="(m,i) in lote.detalles">
                                  <td>{{i+1}}</td>
                                  <td>{{m.caja_inventario.caja.name}}</td>
                                  <td class="td-c"> {{m.caja_inventario.cantidad}}</td>
                                  <td class="td-c"> {{m.caja_inventario.compra}}</td>
                                  <td class="td-c"> {{m.caja_inventario.venta}}</td>
                                  <td class="td-c"> {{m.caja_inventario.cantidad*m.caja_inventario.compra}}</td>
                              ...
                              <tfoot>
                                ...
                                <tr class="table-default">
                                  ...
                                  <td class="text-left bg-white text-black">{{Number(lote.total).toFixed(2)}}</td>
                                </tr>
              ...
              <div v-if="lote.hasOwnProperty('total')" class="col-sm-2 col-12 m-4">
                <div class="form-group">
                  <label for="fullName">Valor Total</label>
                  <input :value="Number(lote.total).toFixed(2)" disabled type="text" class="form-control mb-4">
                </div>
              </div>
              ...
              <div v-if="lote.hasOwnProperty('total')" class="col-sm-2 col-12 m-4">
                <div class="form-group">
                  ...
                  <input :value="Number(lote.monto).toFixed(2)" disabled type="text" class="form-control mb-4">
                </div>
              </div>
              <div v-if="lote.hasOwnProperty('total')" class="col-sm-2 col-12 m-4">
                <div class="form-group">
                  <label for="fullName">Valor Pendiente</label>
                  <input :value="Number(lote.total-lote.monto).toFixed(2)" disabled type="text" class="form-control mb-4">
                </div>
              </div>
              ...
              <div v-if="lote.hasOwnProperty('total')" class="col-sm-2 m-4">
              ...
              <div v-if="lote.hasOwnProperty('total')" class="col-sm-12 col-12">
                ...
                <table class="table">
                  ...
                  <tbody>
                    <tr v-for="m in lote.detalle_pagos">
                      <td>{{m.created_at}}</td>
                      <td>{{m.banco.name}}</td>
                      <td>{{m.monto}}</td>
                    </tr>
              ...
              <div class="col-6" v-if="lote.hasOwnProperty('total')">
                <button v-if="pagar.valor>0&&pagar.valor<=Number(lote.total-lote.monto)" class="btn btn-primary w-100" @click="Save()">Guardar</button>
              </div>
  ...
  createApp({
    ...
    methods: {
      ...
      async GetLote() {
        block.block();
        try {
          let res = await axios.get("{{url('api')}}/compraCajas/" + this.model.compra_id)
          this.lote = res.data
        } catch (e) {
      ...
      async Save() {
        ...
        try {
          ...
          this.pagar.total = this.lote.total
          this.pagar.pendiente = this.lote.total - this.lote.monto
          let res = await axios.post("{{url('api/pagoCompraCajas')}}", this.pagar)
          ...
          swalWithBootstrapButtons({
            title: 'Pago de compra de caja Guardada',
            text: "Su Pago de compra de caja fue guardada",
            type: 'success',
    ...
    mounted() {
      this.$nextTick(async () => {
        ...
        try {
          ...
          await Promise.all([self.GET_DATA('compraCajas'), self.GET_DATA('consolidacionparams'), self.GET_DATA('productos'), self.GET_DATA('almacens'), self.GET_DATA('bancos')]).then((v) => {
            self.compraCajas = v[0]
