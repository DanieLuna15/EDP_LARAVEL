                ...
                methods: {
                    async Save() {
                        try {
                            ...
                            let url = "{{ url('api/almacens') }}";
                            if (this.add == false) {
                                url = "{{ url('api/almacens') }}/" + this.model.id
                    ...
                    async load() {
                        try {
                            ...
                            try {
                                await Promise.all([self.GET_DATA("{{ url('api/almacens') }}"), self.GET_DATA(
                                    "{{ url('api/sucursals') }}"), self.GET_DATA(
                                    "{{ url('api/comprobantes') }}")]).then((v) => {
                                    self.data = v[0]
                                    self.sucursals = v[1]
