                    ...
                    <div class="page-header">
                        <div class="page-title">
                            <p><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                                ...
                                </svg> Almacen / Proveedores de Aves</p>
                        </div>
                        <a href="./proveedors_aves/add" class="btn btn-success">Agregar</a>
                    </div>
                                ...
                                <table id="table_dt" class="table table-hover non-hover" style="width:100%">
                                    ...
                                    <tbody>
                                        <tr v-for="(m,i) in data">
                                            ...
                                            <td>{{ m . telefono }}</td>
                                            <td>{{ m . encargado }}</td>
                                            <td>{{ m . categoria . name }}</td>
                                            <td>
                                                <div class="btn-group">
                                                    <a :href="'./proveedors_aves/edit/' + m.id" class="btn btn-warning btn-sm">Editar</a>
                                                    <button type="button"
                ...
                methods: {
                    async Save() {
                        try {
                            ...
                            let url = "{{ url('api/proveedorComprasAves') }}";
                            if (this.add == false) {
                                url = "{{ url('api/proveedorComprasAves') }}/" + this.model.id
                    ...
                    async load() {
                        try {
                            ...
                            try {
                                await Promise.all([self.GET_DATA("{{ url('api/proveedorComprasAves') }}")]).then((
                                v) => {
                                    self.data = v[0]
                                })
                        ...
                        swalWithBootstrapButtons({
                            ...
                            if (result.value) {
                                try {
                                    ...
                                    let url = "{{ url('api/proveedorComprasAves') }}/" + id
