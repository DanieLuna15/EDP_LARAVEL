...
<body class="white-bg">
            ...
            <td width="60%" valign="middle" style="border:1px solid #ccc;">
                <div style="padding: 8px;">
                    <div
                        ...
                        ENVIO A PP - N° <?= $envioGenPp->id ?>
                    </div>
                    <div style="font-size:12px; text-align:right;">
                        <b>PP N°</b><?= $envioGenPp->Pp->nro ?> </span>
                    </div>
                    <div style="font-size:12px; text-align:right;">
    ...
    <div>
            ...
            <table width="100%" border="0" cellpadding="5" cellspacing="0">
                <tbody>
                    ...
                    <tr>
                        ...
                        <th align="left" class="bold"><strong>LOTE / NRO COMPRA</strong></th>
        ...
        $peso_neto = 0;
        $tara = 0;
                foreach ($envioGenPp->envioGenPpDetalles as $de) {
                    $cajas += $de->detallePp->cajas;
                    $pollos += $de->detallePp->pollos;
                    $peso_bruto += $de->detallePp->peso_bruto;
                    $peso_neto += $de->detallePp->peso_neto;
                    $tara += $de->detallePp->peso_bruto-$de->detallePp->peso_neto;
                ?>
                    <tr class="border_top">
                        <td>{{ $envioGenPp->fecha }}</td>
                        <td>{{ $de->detallePp->LoteDetalle->Lote->Compra->ProveedorCompra->abreviatura }}-{{ $de->detallePp->LoteDetalle->Lote->Compra->nro }}
                            / {{ $de->detallePp->LoteDetalle->Lote->Compra->nro_compra }} </td>
                        <td>{{ $de->detallePp->LoteDetalle->pigmento == 1 ? 'CON PIGMENTO' : 'SIN PIGMENTO' }}</td>
                        <td>{{ $de->detallePp->LoteDetalle->name }}</td>
                        <td>{{ $de->detallePp->cajas }}</td>
                        <td>{{ $de->detallePp->pollos }}</td>
                        <td>{{ $de->detallePp->peso_bruto }}</td>
                        <td>{{ $de->detallePp->peso_bruto - $de->detallePp->peso_neto }}</td>
                        <td>{{ $de->detallePp->peso_neto }}</td>
