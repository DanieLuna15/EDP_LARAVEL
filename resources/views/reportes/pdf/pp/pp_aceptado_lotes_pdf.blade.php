            ...
            <td width="60%" valign="middle" style="border:1px solid #ccc;">
                <div style="padding: 8px;">
                    <div
                        ...
                        ENVÍOS ACEPTADOS EN PP N° <?= $pp->nro ?>
                    </div>
        ...
        <table width="100%" border="0" cellpadding="5" cellspacing="0">
            <tbody>
                ...
                <tr>
                    ...
                    <th align="center" colspan="6" class="bold"><strong>REGISTRO DE TRASPASOS
                            ACEPTADOS</strong></th>
                ...
                <?php
                    ...
                    $peso_neto_t = 0;
                    $tara_t = 0;
                    foreach ($pp->PpTraspasoPps as $de) {
                        $cajas_t += $de->TraspasoPp->cajas;
                        $pollos_t += $de->TraspasoPp->pollos;
                        $peso_bruto_t += $de->TraspasoPp->peso_bruto;
                        $peso_neto_t += $de->TraspasoPp->peso_neto;
                        $tara_t += $de->TraspasoPp->peso_bruto - $de->TraspasoPp->peso_neto;
                    ?>
                <tr class="border_top">

                    <td align="left" class="bold">{{ $de->TraspasoPp->fecha }} /
                        {{ $de->tipo == 1 ? 'PP' : 'PT' }}
                    </td>
                    <td align="left" class="bold">{{ $de->TraspasoPp->cajas }}</td>
                    <td align="left" class="bold">{{ $de->TraspasoPp->pollos }}</td>
                    <td align="left" class="bold">{{ $de->TraspasoPp->peso_bruto }}</td>
                    <td align="left" class="bold">{{ $de->TraspasoPp->peso_bruto - $de->peso_neto }}</td>
                    <td align="left" class="bold">{{ $de->TraspasoPp->peso_neto }}</td>
                ...
                <tr class="border_top">
                    ...
                    <th align="left" class="bold"><strong>TOTALES ACEPTADOS DE PP</strong></th>
