    ...
    <nav id="topbar">
        ...
        <ul class="list-unstyled menu-categories scrollbar" id="topAccordion">
            ...
            <li class="menu single-menu @if($menu['id'] == 1) @endif">
                <a href="#menu-{{ $menu['id'] }}" data-toggle="collapse" aria-expanded="true" class="dropdown-toggle autodroprown">
                    <div class="">
                ...
                <ul class="collapse submenu list-unstyled" id="menu-{{ $menu['id'] }}" data-parent="#topAccordion">
                    @foreach ($menu['sub_menu'] as $subMenu)
                        ...
                        <li class="sub-sub-submenu-list">
                            <a href="#submenu-{{ $subMenu['id'] }}" data-toggle="collapse" aria-expanded="false" class="dropdown-toggle">
                            ...
                            <ul class="collapse list-unstyled sub-submenu" id="submenu-{{ $subMenu['id'] }}" data-parent="#menu-{{ $menu['id'] }}">
                                @foreach ($subMenu['sub_menu'] as $grandChild)
                                <li>
                                    <a href="{{ url($grandChild['route']) }}">
                                        <i class="mdi {{ $grandChild['icon_mdi'] }}"></i> {{ $grandChild['label'] }}
                                    </a>
                                </li>
                                @endforeach
                            </ul>
