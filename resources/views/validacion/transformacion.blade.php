            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-row mb-2">
                            <div class="form-group col-md-12">
                                <label for="inputEmail4">Con Item</label>
                                <select v-model="model.tipo" class="form-control"   name="" id="">
                        ...
                        <div class="form-row mb-2" v-if="model.tipo==1">
                            <div class="form-group col-md-12">
                                <label for="inputEmail4">Item</label>
                                <select v-model="model.item_id" class="form-control"   name="" id="">
                        ...
                        <div class="form-row mb-4" v-if="model.tipo==1">
                            ...
                            <div class="form-group col-md-12">
                                ...
                                <input type="text" :value="promedio" disabled class="form-control" placeholder="0">
                            </div>
                    ...
                    <table id="table_dt" class="table table-hover non-hover" style="width:100%">
                        ...
                        <tbody>
                            <tr v-for="(m,i) in data">
                                <td>{{i+1}}</td>
                                <td>{{m.name}}</td>
                                <td>{{m.tipo==1?'SI':'NO'}}</td>
                                <td>{{m.tipo==1?m.transformacion_item.item.name:''}}</td>
                                <td>{{m.tipo==1?m.transformacion_item.precio:''}}</td>
                                <td>{{m.tipo==1?m.transformacion_item.peso:''}}</td>
                                <td>{{m.tipo==1?m.transformacion_item.promedio:''}}</td>
                                <td>
                                    <div class="btn-group">
                                        <button data-toggle="modal" @click="clickItem(m)" data-target="#exampleModal" type="button" class="btn btn-warning btn-sm">Editar</button>
                                        ...
                                        <div class="dropdown-menu" :aria-labelledby="'menu'+i">
                                            <a class="dropdown-item"  :href="'./transformacion/'+m.id">Sub Items</a>
            ...
            methods: {
                ...
                async Save() {
                    try {
                        ...
                        let url = "{{url('api/transformacions')}}";
                        if (this.add == false) {
                            url = "{{url('api/transformacions')}}/"+this.model.id
                ...
                async load() {
                    try {
                        ...
                        try {
                            await Promise.all([self.GET_DATA("{{url('api/transformacions')}}"),
