@component('application')
                                                            ...
                                                            <template v-for="ptm in pts">
                                                                <template v-for="item in ptm.items">
                                                                    <tr  @click="AddPtItem(item,ptm)"
                                                                        ...
                                                                        <td>{{ Number(item . peso_bruto) . toFixed(3) }} KG</td>
                                                                        <td class="bg-light-info">{{ Number(item . peso_neto) . toFixed(3) }} KG</td>
                                                                    </tr>
                                                                </template>
                                                            </template>
                                                        </tbody>
                                                        ...
                                                        <tbody>
                                                            <template  v-for="m in transformacions">
                                                                <tr class="tr-hover"
                                                                    ...
                                                                    <td>{{ Number( item . total_peso_bruto).toFixed(3) }}</td>
                                                                    <td class="bg-light-info">{{ Number(item . total_peso_neto).toFixed(3) }}</td>
                                                                </tr>
                                                            </template>
                                                        </tbody>
                                                    </table>
    ...
    @slot('script')
                    ...
                    PpItemsModel() {
                        return this.detalle_venta_pp.map((d) => {
                            ...
                            let precio_personalizado = this.ClienteCajacerradas.find(
                                x => x.producto_precio && x.producto_precio.name == "POLLO LIMPIO"
                            );
      ...
                    },
                    ...
                    addVentaPpCarrito(d, i) {
                        if(d.precio_acuerdo > 0){
                            if (this.acuerdo && !this.item_id) {
                                swal.fire({
                                    title: 'Error',
                            ...
                            item.total = this.ModelAcuerdo.total;
                            this.cart_pps.push(item);
                            this.detalle_venta_pp.splice(i, 1);
                        }else{
                         swal({
                    ...
                    addVentaPtCarrito(d, i) {
                        if(d.item.venta > 0){
                            if(d.peso_neto_vender<=d.peso_neto){
                            let item = { ...d }
