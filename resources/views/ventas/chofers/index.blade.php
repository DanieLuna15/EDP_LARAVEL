                    ...
                    <div class="page-header">
                        <div class="page-title">
                            <p><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                                ...
                                </svg> Ventas / Choferes</p>
                        </div>
                        <a href="./chofers/add" class="btn btn-success">Agregar</a>
                    </div>
                                ...
                                <table id="table_dt" class="table table-hover non-hover" style="width:100%">
                                    ...
                                    <tbody>
                                        <tr v-for="(m,i) in data">
                                            ...
                                            <td>{{ m . placa }}</td>
                                            <td>{{ m . zona_despacho. name}}</td>
                                            <td>{{ m . estado_compra_chofer . name }}</td>
                                            <td>{{ m . capacidad }}</td>
                                            <td>
                                                <div class="btn-group">
                                                    <a :href="'./chofers/edit/' + m.id" class="btn btn-warning btn-sm">Editar</a>
                                                    <button type="button"
                                                    ...
                                                    <div class="dropdown-menu" :aria-labelledby="'menu' + i">
                                                        <a :href="'./chofers/turno-apertura/' + m.id"
                                                            class="dropdown-item">Turno</a>
                ...
                methods: {
                    async Save() {
                        try {
                            ...
                            let url = "{{ url('api/chofers') }}";
                            if (this.add == false) {
                                url = "{{ url('api/chofers') }}/" + this.model.id
                    ...
                    async load() {
                        try {
                            ...
                            try {
                                await Promise.all([self.GET_DATA("{{ url('api/chofers') }}")]).then((v) => {
                                    self.data = v[0]
                                })
