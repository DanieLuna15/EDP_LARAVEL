            ...
            methods: {
                async Save() {
                    try {
                        ...
                        let url = "{{url('api/areas')}}";
                        if (this.add == false) {
                            url = "{{url('api/areas')}}/"+this.model.id
                ...
                async load() {
                    try {
                        ...
                        try {
                            await Promise.all([self.GET_DATA("{{url('api/areas')}}")]).then((v) => {
                                self.data = v[0]
                            })
