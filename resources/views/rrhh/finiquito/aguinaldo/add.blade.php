        ...
        <div class="row justify-content-center">
          ...
          <div v-if="personaValidate" class="col-sm-6 col-12 mt-4">
            <div class="widget-content widget-content-area br-6">
              <div class="section general-info">
                <div class="info">
                  <h6 class="">Finiquito Aguinaldo</h6>
                  <div class="row">
                    ...
                    <div class="col-sm-8 col-12">
                      ...
                      <div class="form-group ">
                        <label>Estado</label>
                        <input type="text" :value="contrato.planilla_lista.length<3?'NO APTO PARA FINIQUITO AGUINALDO':'APTO PARA FINIQUITO AGUINALDO'" disabled class="form-control mb-4" placeholder="">
                      </div>
                    </div>
                    <div v-if="contrato.planilla_lista.length>0" class="col-12 ">
                      <div class="row">
                        ...
                        <div class="col-sm-6 col-12">
                          ...
                          <div class="form-group ">
                            <label>Hasta</label>
                            <input type="text" :value="contrato.planilla_lista[contrato.planilla_lista.length>11?11:contrato.planilla_lista.length-1].hasta" class="form-control mb-4" disabled placeholder="">
                          </div>
                        </div>
                      </div>
                    </div>

                    <div v-if="contrato.planilla_lista.length>0" class="col-12 text-center">
                      <hr>
                      <h5 class="text-primary">Valor Finiquito</h5>
                      <h2>{{Number((ContratoPromedio/Number(contrato.planilla_lista.length>11?12:contrato.planilla_lista.length))).toFixed(2)}}</h2>
                    </div>
                    <div v-if="contrato.planilla_lista.length>0" class="col-sm-12 col-12 mb-4">
  ...
  createApp({
    ...
    computed: {
      ...
      ContratoPromedio() {
        ...
        return this.contrato.planilla_lista.reduce((a, b) => a + Number(b.sueldo), 0)
      },
      ValorFaltas() {
    ...
    methods: {
      ...
      async save() {
        ...
        try {
          ...
          this.model.pago = Number((this.ContratoPromedio/Number(this.contrato.planilla_lista.length>11?12:this.contrato.planilla_lista.length)))
          this.model.fecha = this.contrato.planilla_lista[this.contrato.planilla_lista.length > 11 ? 11 : this.contrato.planilla_lista.length - 1].hasta
          this.model.planillas = this.rangeArray(this.contrato.planilla_lista, (this.contrato.planilla_lista.length > 11 ? 12 : this.contrato.planilla_lista.length) - 1)

          let url = "{{url('api/finiaguinaldos')}}";
          ...
          swalWithBootstrapButtons({
            title: 'Finiquito Aguinaldo Guardado',
    ...
    mounted() {
      this.$nextTick(async () => {
        ...
        try {
          ...
          await Promise.all([self.GET_DATA('api/contratos-finiaguinaldos'), self.GET_DATA('api/tipocontratos'), self.GET_DATA('api/areas'), self.GET_DATA('api/costovariables')]).then((v) => {
            self.contratos = v[0]
