        ...
        <div class="page-header">
            <div class="page-title">
                <p><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-grid">
                    ...
                    </svg> Reportes / Liquidacion</p>
            </div>
            <a href="./planillaservicios/add" class="btn btn-success">Agregar</a>
        </div>
            ...
            <div class="form-group ">
              ...
              <div class="input-group mb-4">
                ...
                <div class="input-group-append">
                  <button class="btn btn-primary" @click="Consultar()" type="button">Button</button>
                </div>
              </div>
            </div>
            ...
            <table id="table_dt" class="table table-hover non-hover" style="width:100%">
              ...
              <tbody>
                <tr v-for="(m,i) in data">
                  ...
                  <td>{{Number(m.sueldo_bruto).toFixed(2)}}</td>
                  <td>{{m.fecha}}</td>
                  <td>{{m.user.nombre}}</td>

                  <td>
                    <div class="btn-group">
                      <a :href="m.url_pdf" target="_blank" class="btn btn-warning btn-sm">Editar</a>
        ...
        createApp({
            ...
            methods: {
                async Consultar() {
                    ...
                    try {
                        dt.destroy()
                        await Promise.all([ self.GET_DATA('api/reportes/contratos/liquidacions/'+this.contrato_id)]).then((v) => {
                ...
                async load() {
                    try {
                        ...
                        try {
                            await Promise.all([ self.GET_DATA('api/contratos')]).then((v) => {
