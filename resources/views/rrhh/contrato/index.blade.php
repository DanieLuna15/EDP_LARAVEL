        ...
        <div class="page-header">
            <div class="page-title">
                <p><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-grid">
                    ...
                    </svg> RRHH / Contratos</p>
            </div>
            <a href="./contratos/add" class="btn btn-success">Agregar</a>
        </div>
            ...
            <table id="table_dt" class="table table-hover non-hover" style="width:100%">
              ...
              <tbody>
                <tr v-for="(m,i) in data">
                  <td>{{i+1}}</td>
                  <!-- <td>{{m.persona.nombre}} {{m.persona.apellidos}}</td>
                  <td>{{m.persona.documento.name}} {{m.persona.doc}}</td>
                  <td>{{m.area.name}}</td>
                  <td>{{m.tipocontrato.name}}</td>
                  ...
                  <td>{{m.tipocontrato.name}}</td>
                  <td>{{m.sucursal.nombre}}</td>
                  <td>{{m.servicio==1?'NO':'SI'}}</td>
                  <td>{{m.inicio}}</td>
                  <td>{{m.fin}}</td>
                  <td>{{m.sueldo}}</td>
                  <td>{{m.sueldo_bruto}}</td>

                  <td>
                    <div class="btn-group">
                      <a :href="'./contratos/edit/'+m.id" class="btn btn-warning btn-sm">Editar</a>
                      ...
                      <div class="dropdown-menu" :aria-labelledby="'menu'+i">
                        <a class="dropdown-item" target="_black" :href="m.url_pdf">PDF</a>
            ...
            methods: {
                async Save() {
                    try {
                        ...
                        if (this.add == false) {
                            url = "{{url('api/contratos')}}/"+this.model.id
                ...
                async load() {
                    try {
                        ...
                        try {
                            await Promise.all([self.GET_DATA("{{url('/api/contratos')}}")]).then((v) => {
                                self.data = v[0]
                            })
