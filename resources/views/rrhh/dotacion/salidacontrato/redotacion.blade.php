...
<div class="row">
              ...
              <div class="card-body">
                ...
                <div class="user-info">
                  ...
                  <div class="card-star_rating">
                    ...
                    <span class="badge badge-success mt-2 w-100">Cantidad {{m.stock_inventario}}</span>
                  </div>
                  <button @click="Agregar(m)"  :disabled="m.stock_inventario<=0"   class="btn btn-warning mt-4 btn-sm w-100">DOTAR</button>
                </div>
              </div>
            </div>
          </div>
  ...
  <div class="col-6">
    ...
    <div class="col-12 mb-2" v-for="(m,i) in dotaciones">
          ...
          <div class="media m-0">
            ...
            <div class="media-body">
              ...
              <p class="media-text">
                ...
                <span class="badge badge-success mr-2">Stock {{Number(m.cant).toFixed(2)}}</span>
              ...
              <div class="btn-group" role="group" aria-label="Basic example">
                  <button  data-toggle="modal" data-target="#exampleModal" type="button" class="btn btn-primary"  @click="dotacion_edit=m,edit=true">
                  Cantidad {{Number(m.cantidad)}}
                  </button>
                  <button type="button"  @click="m.cantidad--" :disabled="m.cantidad<=0" class="btn btn-dark">
        ...
        <div class="col-12 mb-2" v-for="(m,i) in model.detalle">
          <div class="widget-content widget-content-area notation-text-icon">

            <div class="media m-0">

              <div class="media-body">
                <h4 class="media-heading">{{m.stockdotaciondetail.dotacion.name}}</h4>
                <p class="media-text">
                  <span class="badge badge-danger mr-2">Compra {{Number(m.stockdotaciondetail.dotacion.costo).toFixed(2)}}</span>
                  <span class="badge badge-primary mr-2">Venta {{Number(m.stockdotaciondetail.dotacion.venta).toFixed(2)}}</span>
                  <span class="badge badge-success mr-2">Cantidad {{parseInt(m.cantidad)}}</span>
          ...
          <div class=" col-12 ">
            <div class="widget-three">
              <div class="widget-content">
                ...
                <button @click="Save()" class="btn btn-block btn-primary mt-4"> GUARDAR REDOTACION </button>
              </div>
            </div>
          </div>
        </div>
    ...
    methods: {
      Agregar(param) {
        ...
        if(buscar.length==0){
          this.dotaciones.push({
            ...
            dotacion_costo: dotacion.costo,
            dotacion_venta: dotacion.venta,
            cant: dotacion.stock_inventario,
      ...
      async load() {
        try {
          ...
          try {
            await Promise.all([self.GET_DATA('api/salidadotacioncontratos/{{$id}}'), self.GET_DATA('api/clientes'),self.GET_DATA('api/stockdotacions/sucursal/1'),]).then((v) => {
              self.model = v[0]
              self.data = v[2]
              self.clientes = v[1]
            })
