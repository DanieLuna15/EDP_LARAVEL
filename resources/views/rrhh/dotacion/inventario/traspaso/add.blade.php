              ...
              <div class="card-body">
                ...
                <div class="user-info">
                  ...
                  <div class="card-star_rating">
                    ...
                    <span class="badge badge-success mt-2 w-100">Stock {{m.stock_inventario}}</span>
                    <span class="badge badge-warning mt-2 w-100">Lote: {{m.lote}}</span>
                  </div>
                  <button @click="Agregar(m)" class="btn btn-dark mt-4 btn-sm w-100">TRASPASAR</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="widget-content widget-content-area notation-text-icon">
          ...
          <div class="media m-0">
            ...
            <div class="form-group col-md-6">
              ...
              <input type="text" v-model.number="m.dotacion.costo" class="form-control" placeholder="Costo">
            </div>
            <div class="form-group col-md-6">
              <label for="inputEmail4">Venta</label>
              <input type="text" v-model.number="m.dotacion.venta" class="form-control" placeholder="Venta">
            </div>
            <div class="form-group col-md-12">
              <label for="inputEmail4">Cantidad</label>
              <input type="text" v-model.number="m.cantidad" class="form-control" placeholder="Cantidad" >
            </div>
      ...
      <div class=" col-12 layout-spacing">
        <div class="widget-three">
          <div class="widget-content">
            ...
            <div class="form-group col-md-12">
              <label for="inputEmail4">Sucursal Destino</label>
              <select v-model="sucursal_destino_id" name="" class="custom-select" id="">
                <template  v-for="(m,i) in sucursals" >
                    <option v-if="sucursal.id!=m.id" :value="m.id">{{m.nombre}}</option>

                </template>
              </select>
            </div>
            </div>
            <button @click="Save()" class="btn btn-block btn-primary mt-4"> GUARDAR TRASPASO </button>
          </div>
        </div>
      </div>
    </div>
        ...
        <div class="modal-body">
          ...
          <div class="form-row mb-4" v-else>
            ...
            <div class="form-group col-md-12">
              ...
              <select v-model="sucursal_destino_id" name="" class="custom-select" id="">
                <option v-for="(m,i) in sucursals" :value="m">{{m.nombre}}</option>
              </select>
            </div>
...
<script type="module">
    ...
    methods: {
      ...
      Agregar(param) {
        ...
        let buscar = this.dotaciones.filter((v,i)=>param.id == v.dotacion.id);
        if(buscar.length==0){
          this.dotaciones.push({
            dotacion: {
              ...
              stock: dotacion.stock_inventario,
              lote: dotacion.lote
            },
            cantidad: 0,
            proveedor: {}
      ...
      async Save() {
        ...
        try {
          ...
          this.model.dotaciones = this.dotaciones
            ...
            this.model.sucursal_destino_id = this.sucursal_destino_id
            this.model.user_id = this.user_id
          let url = "{{url('api/traspasoDotacions')}}";
      ...
      async load() {
        try {
          ...
          try {
            await Promise.all([self.GET_DATA('api/stockdotacions/sucursal/'+this.sucursal.id), self.GET_DATA('api/proveedors')]).then((v) => {
              self.data = v[0]
              self.proveedors = v[1]
            })
    ...
    mounted() {
      this.$nextTick(async () => {
        ...
        try {
          ...
          await Promise.all([self.load(), self.GET_DATA('api/sucursals') ]).then((v) => {
            this.sucursals = v[1]
          })
          dt.create()
