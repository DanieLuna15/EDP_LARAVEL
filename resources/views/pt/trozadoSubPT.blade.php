        ...
        <div class="page-header">
            <div class="page-title">
                <p><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-grid">
                    ...
                    </svg> Sub Descomposicion de Detalle de PT</p>
            </div>
            ...
            <table id="table_dt" class="table table-hover non-hover" style="width:100%">
              <thead>
                <tr>
                  ...
                  <th>Usuario</th>
                  <th>Producto</th>
                  <th>Descomposicion</th>
                  <th>Sub Descomposicion</th>
                  <th>Cantidad </th>
                  <th>Peso Total</th>
              ...
              <tbody>
                <tr v-for="(m,i) in data">
                  <td>{{m.lote.fecha}} #{{m.id}}</td>
                  <td>{{m.lote.compra.nro}}</td>
                  <td>{{m.lote.compra.fecha}}</td>
                  <td>{{m.lote.user.nombre}}</td>
                  <td>{{m.pt_detalle.lote_detalle.name}}</td>
                  <td>{{m.pt_detalle_descomposicion.compo_externa.name}}</td>
                  <td>{{m.compo_externa_detalle.name}}</td>
                  <td>{{m.cantidad}}</td>
                  <td>{{m.peso_total}}</td>
                  ...
                  <td>
                    <div class="btn-group">
                      ...
                      <a :href="m.url_lote_pdf" target="_blank" class="dropdown-item">Ver Lote PDF</a>
            ...
            methods: {
                ...
                async load() {
                    try {
                        ...
                        try {
                            await Promise.all([self.GET_DATA("{{url('api/loteEnvioPts')}}")]).then((v) => {
                                self.data = v[0]
                            })
                ...
                async ConsultarFecha(){
                    ...
                    try {
                        ...
                        dt.destroy()
                        let res = await axios.post("{{url('api/subDesPtDetalleDescosFecha')}}",data).then((v) => {
                                self.data = v.data
                            })
