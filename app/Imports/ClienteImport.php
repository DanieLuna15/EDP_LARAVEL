...
class ClienteImport implements ToModel
{
    ...
    // //         $existingClient = Cliente::where('doc', $row[2])->first();
    // //         if ($existingClient) {
    // //             $errors[] = "El número de documento ya existe: " . $row[2];
    // //         }
    // //         // Validaciones y registro de errores si faltan datos
    // //         if (!$Documento) Log::error("Documento no encontrado para: " . $row[1]);
    // //         if (!$Tipocliente) Log::error("Tipo de cliente no encontrado para: " . $row[3]);
    // //         if (!$GrupoCliente) Log::error("Grupo cliente no encontrado para: " . $row[4]);
    // //         if (!$Preventista) Log::error("Preventista no encontrado para: " . $row[23]);
    // //         if (!$AcuerdoCliente) Log::error("Acuerdo cliente no encontrado para: " . $row[24]);
    // //         if (!$Tipopago) Log::error("Tipo de pago no encontrado para: " . $row[25]);
    // //         if (!$TipoNegocio) Log::error("Tipo de negocio no encontrado para: " . $row[26]);
    // //         if (!$FormaPedido) Log::error("Forma de pedido no encontrado para: " . $row[27]);
    // //         if (!$Chofer) Log::error("Chofer no encontrado para: " . $row[28]);
    // //         if (!$ZonaDespacho) Log::error("Zona de despacho no encontrada para: " . $row[29]);

    // //         // Si algún dato esencial falta, retornamos null y no guardamos el cliente
    // //         if (!$Documento || !$Tipocliente || !$GrupoCliente || !$Preventista || !$AcuerdoCliente || !$Tipopago || !$TipoNegocio || !$FormaPedido || !$Chofer || !$ZonaDespacho || $existingClient) {
    // //             Log::error("Error: Faltan datos de referencia para el cliente: ", $row);
    // //             return null; // No guardar cliente si falta información crítica
    // //         }

    // //         // Asignación de valores con validación de campos
    // //         $tipos = [
    // //             "" => "0",
    // //             "POR MAYOR" => "5",
    // //             "OFERTA" => "6",
    ...
    // //         // Crear cliente (este paso solo se realiza si todo es válido)
    // //         $cliente = new Cliente();
    // //         $cliente->nombre = $row[0];
    // //         $cliente->documento_id = $Documento ? $Documento->id : 1; // Si no se encuentra, asignar 1
    // //         $cliente->doc = $row[2];
    // //         $cliente->tipocliente_id = $Tipocliente->id ?? 1;  // Asignar 1 si no se encuentra
    // //         $cliente->cinta_cliente_id = $GrupoCliente->id ?? 1;
    // //         $cliente->caja_cerrada = $row[5] == 'SI' ? 1 : 0;
    // //         $cliente->telefono = $row[6];
    // //         $cliente->direccion = $row[7];
    // //         $cliente->correo = $row[8];
    ...
    public function model(array $row)
    {
        ...
        if ($row[0] != 'NOMBRE') {
            ...
            $GrupoCliente = CintaCliente::where([['name', $row[4]], ['estado', 1]])->first();
            $Preventista = User::where([['nombre', $row[23]], ['estado', 1]])->first();
            $Distribuidor = User::where([['nombre', $row[35]], ['estado', 1]])->first();
            $Tipopago = Tipopago::where([['name', $row[25]], ['estado', 1]])->first();
            $TipoNegocio = TipoNegocio::where([['name', $row[26]], ['estado', 1]])->first();
            $FormaPedido = FormaPedido::where([['name', $row[27]], ['estado', 1]])->first();
            $Chofer = Chofer::where([['nombre', $row[28]], ['estado', 1]])->first();
            ...
            if (!$GrupoCliente) Log::error("Grupo cliente no encontrado para: " . $row[4]);
            if (!$Preventista) Log::error("Preventista no encontrado para: " . $row[23]);
            if (!$Distribuidor) Log::error("Distribuidor no encontrado para: " . $row[35]);
            //if (!$AcuerdoCliente) Log::error("Acuerdo cliente no encontrado para: " . $row[24]);
            if (!$Tipopago) Log::error("Tipo de pago no encontrado para: " . $row[25]);
            if (!$TipoNegocio) Log::error("Tipo de negocio no encontrado para: " . $row[26]);
            if (!$FormaPedido) Log::error("Forma de pedido no encontrado para: " . $row[27]);
            ...
            // Si algún dato esencial falta, retornamos null y no guardamos el cliente
            if (!$Documento || !$Tipocliente || !$GrupoCliente || !$Preventista || !$Tipopago || !$TipoNegocio || !$FormaPedido || !$Chofer || !$ZonaDespacho || $existingClient) {
                Log::error("Error: Faltan datos de referencia para el cliente: ", $row);
                return null; // No guardar cliente si falta información crítica
            }
            ...
            $cliente->creditos_activos = $row[10];
            $cliente->dias_horas = $row[11];
            $cliente->latitud = ($row[12] !== null && $row[12] !== '')
                ? $this->formatCoord($row[12])
                : 0;

            $cliente->longitud = ($row[13] !== null && $row[13] !== '')
                ? $this->formatCoord($row[13])
                : 0;
            $cliente->horario_preferencia = $row[14];
            $cliente->horario_pedido = $row[15];
            $cliente->dinero_cuenta = $row[16];
            ...
            if (strtoupper(trim($row[24])) === 'NINGUNO') {
                $acuerdoId = 0;
            } else {
                $AcuerdoCliente = AcuerdoCliente::where([['name', $row[24]], ['estado', 1]])->first();
                $acuerdoId = $AcuerdoCliente->id ?? 0;
            }

            $cliente->acuerdo_cliente_id = $acuerdoId;
            $cliente->tipopago_id = $Tipopago->id ?? 1;
            $cliente->tipo_negocio_id = $TipoNegocio->id ?? 1;
            $cliente->forma_pedido_id = $FormaPedido->id ?? 1;
            $cliente->chofer_id = $Chofer->id ?? 1;
            ...
            $cliente->aprobado = $row[33] == 'SI' ? 1 : 0;
            $cliente->activo = $row[34] == 'SI' ? 1 : 0;
            $cliente->distribuidor_id = $Distribuidor->id ?? 1;
            // Guardar cliente
            $cliente->save();
    ...
    private function formatCoord($value)
    {
        if ($value === null || $value === '') return null;
        $s = trim((string) $value);
        $s = str_replace(',', '.', $s);

        if (preg_match('/^-?\d+(\.\d+)?$/', $s) && strpos($s, '.') !== false) {
            return (float) $s;
        }
        $neg = false;
        if (strlen($s) > 0 && $s[0] === '-') {
