...
class ClienteImport implements ToModel
{
    ...
    public function model(array $row)
    {
        ...
        try {
            $errors = [];  // Arreglo para almacenar los errores de las filas

            // Comenzamos la validación de cada fila
            if ($row[0] != 'NOMBRE') {
                ...
                // Si hay errores, lanzamos una excepción y no guardamos nada
                if (count($errors) > 0) {
                    // Log de errores
                    Log::error("Errores en la fila: " . implode(", ", $errors));
                    // Aquí lanzamos la excepción, que permitirá salir del proceso sin guardar nada
                    throw new \Exception("Faltan datos de referencia para la fila: " . implode(", ", $errors));
                }

                // Asignación de valores y creación del cliente
                $tipos = [
                    "" => "0",
                    "POR MAYOR" => "5",
                    "OFERTA" => "6",
                ...
                Log::info("Pre Save: " . json_encode($cliente->toArray()));
                $cliente->save();
                Log::info("Post Save (ID): " . $cliente->id);

                Log::info("Antes de commit para cliente: " . $cliente->id);
                DB::commit();
                Log::info("Después de commit para cliente: " . $cliente->id);
            ...
            DB::rollBack();
            Log::error("Rollback ejecutado para fila: " . json_encode($row));
        Log::error("Error al importar cliente: " . $e->getMessage());

            // Lanzar la excepción, para que el frontend la maneje y muestre el error
